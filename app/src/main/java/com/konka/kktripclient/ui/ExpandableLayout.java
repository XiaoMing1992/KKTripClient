...
public class ExpandableLayout extends FrameLayout {
    ...
    private void init(AttributeSet attrs) {
        if (attrs != null) {
            ...
            initState = a.getFloat(R.styleable.ExpandableLayout_el_init_state, 0);
            orientation = a.getInt(R.styleable.ExpandableLayout_android_orientation, VERTICAL);
            expansion = a.getBoolean(R.styleable.ExpandableLayout_el_expanded, false) ? 1 : initState;
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        expansion = isExpanded() ? 1 : initState;
    ...
    private void setExpanded(boolean expand, boolean animate) {
        ...
        if (!expand && (state == COLLAPSING || expansion == initState)) {
            return;
        }

        float targetExpansion = expand ? 1 : initState;
        if (animate) {
    ...
    private void animateSize(final float targetExpansion) {
        ...
        animator.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationStart(Animator animation) {
                state = targetExpansion == initState ? COLLAPSING : EXPANDING;
            }
